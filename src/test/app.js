  ...
  describe('GET /', function() {
    it('should see title "Posts"', function(done) {
      request.get('/').expect(200, function(err, res) {
        ...
        res.should.be.html
        res.text.should.containEql('<title>Posts</title>')
    ...
    it('should see 0 post', function(done) {
      request.get('/').expect(200, function(err, res) {
        ...
        res.should.be.html
        res.text.should.containEql('<p>You have <strong>0</strong> posts!</p>')
        done()
      })
    })
  })
  ...
  describe('GET /post/0', function() {
    it('should see post', function(done) {
      request.get('/post/0').expect(200, function(err, res) {
        ...
        res.should.be.html
        res.text.should.containEql('<h1>Title</h1>')
        res.text.should.containEql('<p>Contents</p>')
        done()
      })
    })
  })
